...
Es gibt von efaLive zum einen ein DVD Abbild, eine so genannte Live-DVD, und ein
...
\bigskip
\textbf{ACHTUNG:} Auch wenn die hier beschriebene efaLive-DVD die Festplatte des
...
Live-DVD in irgendeiner Weise beeinflusst wird. Ich möchte ausdrücklich
darauf hinweisen, dass die Benutzung der DVD auf eigene Gefahr erfolgt
...
\section{efaLive-DVD}
\label{sct:efalivecd}
Eine Live-DVD ist eine DVD, von der Computer
gestartet werden können. Das System läuft komplett von der DVD und lässt
die Festplatte unangetastet. Daher eignen sich solche Live-DVDs
...
Ein weiteres Merkmal von Live-DVDs ist, dass alle Änderungen, die während
des Betriebs vorgenommen wurden, nach dem Herunterfahren des Systems
verloren gehen. Da die Festplatte nicht angetastet wird und eine DVD-ROM
...
Dies sind die Hardwareanforderungen, wenn man die Live-DVD benutzen
möchte. Die Angaben sind als Minimalvoraussetzung zu verstehen.

\begin{itemize}
    \item Intel Pentium III Prozessor mit 600MHz
    \item 768 MB Arbeitsspeicher
    \item DVD-ROM Laufwerk oder USB Anschluss
    \item Monitor mit einer Auflösung von 1024x768 Pixeln
    \item ggf. USB Anschluss für die Datensicherung
\end{itemize}

\subsection{efaLive-DVD erstellen}
\label{sct:cd_erstellen}
efaLive kann im Internet unter \cite{EFA4} als ISO DVD Abbild
heruntergeladen werden. Um aus dem DVD Abbild eine DVD zu erstellen,
bieten fast alle Brennprogramme einen Menüpunkt "`Abbild brennen"', 
"`Write DVD image"' oder ähnlich. Für weitere Informationen bitte
...
Ansonsten gelten die gleichen Bedingungen, wie bei einer DVD.
...
\begin{minipage}{\linewidth}
    ...
    \captionof{figure}{Auswahlbildschirm Bootloader (Syslinux)}
    \label{fig:syslinux}
\end{minipage}
\bigskip
...
\begin{enumerate}
    \item DVD in das DVD-ROM Laufwerk des Computers einlegen oder USB Stick
        einstecken
...
\begin{minipage}{\linewidth}
    ...
    \captionof{figure}{efaLive-Setup}
...
aus dem Verzeichnis \texttt{snapshot} der DVD auf diesen entpacken, so dass im
...
danach mit der efaLive-DVD starten. Während des Startvorgangs sollte der
...
\begin{itemize}
    ...
    \item DVD-ROM Laufwerk oder USB Anschluss (nur für Installation)
    \item Monitor mit einer Auflösung von 1024x768 Pixeln
    \item ggf. USB Anschluss für die Datensicherung
\end{itemize}
...
Desktop-PC mit einer Festplatte, einem DVD-Rom Laufwerk und optional
...
Die Screenshots von der Installtion sind von einer älteren Version von
Debian, aber inhaltlich gleich.
...
gestartet. Damit nicht wieder von der Live-DVD gestartet wird, empfiehlt
es sich, diese nun aus dem DVD-ROM Laufwerk zu nehmen, bzw. den USB
Stick zu entfernen.
...
Live-DVD, kann das Bild, welches während des Starts angezeigt wird, mit
...
Das efaLive-Abbild für den RspberryPi läuft auf den Modellen 1 bis 4.
...
des RaspberryPi gesteckt werden. Der Pi startet zuerst selbstständig neu, bei dem RaspberryPi 4 muss man manchmal
noch einmal manuell neu starten. Danach sollte direkt efaLive-Setup angezeigt werden. Zuerst sollten nun die 
Spracheinstellungen in raspi-config vorgenommen werden. Die Sprache (z.B. de\_DE.UTF8), die Zeitzone, 
die Tastatursprache und das Land für das WLAN sollte hier konfiguriert werden. Nach einem Neustart über den Tab "`Aktionen"' greifen die Einstellungen.

Das System kann nun weiter wie gewohnt eingerichtet werden.


\subsection{Hardware-Uhr}
\label{sct:raspi_hardware_clock}
Der RaspberryPi kann mit einer Hardware-Uhr ausgestattet werden, damit nach einem Neustart die
Systemzeit wieder richtig gesetzt wird, falls keine Internetverbindung zur Verfügung steht. Um
die Hardware-Uhr zu aktivieren, muss als Benutzer "`root"' eine Zeile in der Datei \texttt{/boot/config.txt} aktiviert
werden (\# entfernen). Je nach Modell der Uhr eine der beiden Zeilen:
\bigskip
\\
...
\bigskip
\\
Außerdem muss die Synchronisation der Systemuhr mit der Hardware-Uhr aktiviert werden. Dies geht mit
dem Befehl:
\bigskip
\\
...
\bigskip
\\
Nach einem Neustart sollte die Hardware-Uhr aktiviert werden.


\subsection{Ein-/Ausschalter}
\label{sct:raspi_power_button}
Ein RaspberryPi kann mit einem Schalter eingeschaltet werden. Für das Ausschalten reicht es ggf. efa
zu beenden. Zum Einschalten muss ein Schalter genutzt werden. Dies sollte ein Taster sein, der beim Drücken Pin
5 (GPIO3) und 6 (GND) des RaspberryPi kurzschließt.
...
Optional können die Datensicherungen verschlüsselt werden.
...
Handelt es sich um eine verschlüsselte Datensicherung, muss zuerst das zugehörige Passwort
in efaLive-Setup konfiguriert werden.
...
\begin{figure}
    \centering
    \includegraphics[width=5cm]{screenshots/efalive_setup_devices_dialog.png}
...
\begin{minipage}{\linewidth}
    \centering
    \captionsetup{type=figure}
    \includegraphics[width=3cm]{screenshots/efalive_setup_backup_dialog.png}
...
\paragraph{W-LAN}
\label{sct:wifi}
Um eine W-LAN Verbindung hinzuzufügen, auf den Knopf "`+"' klicken
...
\begin{minipage}{\linewidth}
    \centering
    \captionsetup{type=figure}
    \includegraphics[width=7cm]{screenshots/efalive_setup_date_dialog.png}
...
zu der aktuell ausgewählten Aufgabe verändert werden. Über den "`Play'"-Knopf
kann die aktuell ausgewählte Aufgabe testweise ausgeführt werden. Etwaige Ausgaben
erscheinen in der Logdatei.
...
Sämtliche über das efaLive System erstellten Backups können verschlüsselt werden.
Dazu kann hier der entsprechende Haken gesetzt und ein Passwort vergeben werden.
Um ein solches Backup auf einem Linux System zu entschlüsseln kann folgender
Befehl genutzt werden: \texttt{gpg -d -o some-backup.zip some-backup.zip.gpg}.
Unter Windows kann z.B. Gpg4Win benutzt werden.
...
Hier können Einstellungen für das von efaLive zu verwendende E-Mail-Konto
...
Das Feld "`Absender"' ist mit der E-Mail-Adresse des gewünschten Absenders zu
füllen.
